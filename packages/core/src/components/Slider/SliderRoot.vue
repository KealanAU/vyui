...
export interface SliderRootProps extends PrimitiveProps, FormFieldProps {
   ...
   * `number` (e.g. `50`) or `[50]`; multi-thumb is `number[]`. Emits in
   * whichever shape the input arrived in — pass `50`, get `50` back.
   */
...
export type SliderRootEmits = {
  /** Event handler called when the slider value changes. Payload shape mirrors
   *  the input — `number` if `modelValue` was a number, else `number[]`. */
  'update:modelValue': [payload: number | number[] | undefined]
  /**
   * Event handler called when the value changes at the end of an interaction —
   * e.g. to update a backend service. Payload shape mirrors the input.
   */
...
// Emit in whichever shape the consumer is binding with, detected per call so a
// parent that reassigns the ref shape mid-life still works.
...
// writes to `.current` are a silent no-op. Every later sync hops through a
// setter worklet, fired from post-mount `watch` callbacks so they can't hit the
// setup-time race against MT-ref registration.
 ...
 * snapped, clamped, sorted and honoured `minStepsBetweenThumbs`, so a failure
 * here means a stale MT mirror rather than routine filtering.
 */
...
// against this, not the previous frame — the live updates have already written
// the new value by the time the commit lands.
 ...
 *
 * The checks below are a backstop against a stale MT mirror, not routine
 * filtering. They must stay cheap and total: a rejection strands the MT-painted
 * fill, which has no way to learn the commit didn't land.
 */
function commitFromMT(nextValues: number[]) {
  ...
  // updates usually got there first), the commit against the gesture's start.
  const prev = currentModelValue.value
  if (nextValues.some((v, i) => v !== prev[i]))
    writeModelValue(nextValues)
