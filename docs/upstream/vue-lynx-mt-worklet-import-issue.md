...
- When the MT loader processes a `.vue` SFC or a `.ts` file that *is* reached via a non-relative import, it produces an MT module whose import list is empty, because the aliased dependency is simply absent from the MT graph.
...
- Therefore, no combination of `sideEffects: false`, `/*#__PURE__*/` annotations, or bundler DCE settings can rescue the registration, because the code path is excluded earlier, at import extraction.
...
- Change **only** the import specifier to a relative path (`import { useGesture } from './gesture'`), with no other code changes, and rebuild → worklet registers and runs correctly.
...
- A minimal variant: also re-emit specifiers matching `@/…` / `~/…` alias patterns, since the downstream bundler already knows how to resolve them, so just not filtering them out is enough.
