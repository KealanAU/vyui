...
1. **Verified bug:** opacity modifiers on semantic colors (`bg-neutral-900/50`,
   `bg-neutral-100/50`) generate **no CSS at all**, so every modal/drawer/tray overlay and
...
`@vyui/core` emits a single bare mode-tier utility, and every component resolves the ramp
...
that actually breaks silently, since every style must declare every token the base declares, so
...
- ~~`button.ts:19` "No dark mode"~~ is **fixed in #129.** Now reads "Dark mode rides the
...
- **Island family, STILL NOT MIGRATED:** `island.ts` (`bg-white/80` + `border-black/5`),
  `islandButton.ts` (`text-slate-700/900`, raw slate, bypassing neutral entirely).
...
`text-white` on solid color fills, since a -500 solid reads fine on both modes.
...
- `avatarGroup.ts` overlap-ring `border-white`, an intentional gap pending a `ring-bg`
  equivalent (§5.1).
...
- In dark mode, neutral **classes** flip via the inverted ramp but neutral **icon hex**
  does not, so `text-neutral-700` icons resolve to slate-700 and go dark-on-dark.
...
Commit 9490d0f ("bake icon color hex") added a per-consumer *workaround* instead, where the
...
9. Write `docs/styling.md` to promote the conventions that currently live only in file
...
- **Alpha:** no `/<alpha>` on semantic colors; use discrete shades (`/10`→`-50`,
