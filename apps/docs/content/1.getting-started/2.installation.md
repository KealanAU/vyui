...
Override the semantic color slots (`primary`, `secondary`, `success`, `info`, `warning`, `error`, `neutral`) by redefining the `--ui-color-{semantic}-{shade}` CSS variables in your own stylesheet, or add a brand-new color slot (e.g. `tertiary`) with `defineVyuiConfig` — see [Theming → Configuring colors](/theming#configuring-colors) for the full walkthrough, including the type-safety and codegen steps.
