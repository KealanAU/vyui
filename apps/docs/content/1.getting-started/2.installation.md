...
Add `VyUI` to your app entry once. It provides the theme config and registers every `Vy*` component globally. Kit's defaults use `lucide` icons, so register that icon set up front, and install the `Intl` polyfill for Lynx's PrimJS engine.
...
createApp(App).use(VyUI).mount()
```

::tip
Prefer to keep your bundle lean? `provideVyUI(app, options)` supplies the theme config **without** registering all components — pair it with named imports (`import { VyButton } from '@vyui/kit'`) so the bundler only pulls the components you use. `app.use(VyUI, { components: { VyButton } })` registers a chosen subset globally.
::
...
| Vue-Lynx / minimal | `provideVyUI(app, config)` | **Local imports** (`import { VyButton } from '@vyui/kit'`) |
