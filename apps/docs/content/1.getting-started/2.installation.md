...
worklet-driven component runs — dragging a `VySortable`, a gesture, an
animation:
...
`npx @vyui/cli check` — it audits your config and reports this (and other common
wiring gaps) with the fix, and exits non-zero so you can gate CI on it. `init`
adds the allowlist for you on new projects.
