...
`@vyui/core` ships an `<Icon>` primitive that accepts any [Iconify](https://icones.js.org) name. There is no bundled icon set, so pick a collection, install the `@iconify-json` package, and use it.
