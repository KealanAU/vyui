...
4. **You own the result**, since added components are plain source files. Re-running `add` preserves them by default (pass `--overwrite` to refresh), and the shared library and transitive dependencies are never silently overwritten.
...
- allowlists `@vyui/core`'s main-thread worklets in `lynx.config.*` (`includeWorkletPackages`) so gestures work at runtime,
- pulls the theme tokens into your global CSS,
- installs the shared runtime dependencies.

Anything it can't confirm, such as a non-standard entry or a missing Tailwind config, is printed as a manual step instead of being guessed. Use `--dry-run` to preview every file and edit first, and `info` afterwards to confirm what was detected.
...
Audit an initialised project for the common wiring gaps that only bite at runtime, most often a `lynx.config.*` missing `includeWorkletPackages`, which crashes the first gesture with `TypeError: cannot read property 'bind' of undefined`. It's read-only and offline: it reports each gap with its fix and exits non-zero, so you can gate CI on it. Run `init --overwrite` (or fix manually) to resolve what it finds.

::code-group
```bash [pnpm]
pnpm dlx @vyui/cli check
```

```bash [yarn]
yarn dlx @vyui/cli check
```
