...
The default theme is a port of Nuxt UI's card, using vyui's semantic surface/border tokens so it adapts automatically under dark mode (see [Theming → Dark Mode](/theming/dark-mode)). The default variant is `outline`.
