...
The `side` variant sets `justify-start` or `justify-end` on `actions`. The default theme uses the `bg-default` and `bg-elevated` surface tokens for the foreground and action wrapper, so it adapts automatically under dark mode (see [Theming → Dark Mode](/theming/dark-mode)).
