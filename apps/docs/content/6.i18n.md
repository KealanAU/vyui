...
- **Overridable built-in strings.** Every library-owned label (close, clear) resolves through a small message map you can replace per locale, so a French app gets "Suivant" instead of "Next Page".
- **Richer formatting helpers** would replace the current English-only fallback formatter with a real locale-aware layer.
- **Complete RTL coverage** would make layout direction-aware for Arabic, Hebrew, and similar, so start/end spacing and icon flipping resolve consistently from `dir`.
...
The remaining work is the overridable string map, richer formatting, locale-aware accessibility defaults, and full RTL coverage. See the [roadmap](/getting-started/roadmap).
