---
title: Accessibility
description: Vy UI components ship with native Lynx accessibility built in, covering what you get for free and the one thing you add.
navigation:
...
Drop a component in and the native accessibility data is already on the element. State stays in sync automatically, so a checkbox announces "checked" / "unchecked" as it toggles, an accordion "expanded" / "collapsed", a slider its value. This is true whether you use the headless `@vyui/core` primitives or the styled `@vyui/kit` components:
...
A component can't guess a name for a control that has no text, such as an icon-only button, an avatar, or a close "×". Give those an `accessibility-label` (Lynx's equivalent of `aria-label`) so screen readers announce something meaningful:
...
::note
Lynx allows one role per element, so a `disabled` control announces "disabled" rather than its usual role. For tabs and options, pass `selected` rather than a `state` string, since selected items announce "selected" and unselected ones stay quiet, which is how a screen reader expects a list to behave.
::
