...
Vy UI uses this channel itself: keyboard awareness (`keyboardstatuschanged`), lazy mounting (`exposure` / `disexposure`), and live theme changes (`themechanged`. See [useColorMode](/composables/use-color-mode)). [Sparkling](/guides/sparkling) handles the sending side for you; if you are building a custom host, the [native host integration guide](/guides/native-host-integration) shows the Swift and Kotlin.
