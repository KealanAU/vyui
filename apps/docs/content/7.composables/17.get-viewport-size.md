...
`SystemInfo` is a static snapshot that doesn't update on rotation or split screen. Re-read it when layout actually changes: [`VyApp`](/components/app) emits `viewport-change` from the root `layoutchange` event for exactly this.
...
`getViewportSize(): ViewportSize | null` returns `{ width: number, height: number }` in logical px.
