...
- `isDark` — resolved `ComputedRef<boolean>`. Folds `'system'` to the OS appearance: on web via `matchMedia`, on Lynx native via the host's `theme` global prop and `themechanged` global event (see [native host integration](/guides/native-host-integration)). A host that injects neither resolves to light.
