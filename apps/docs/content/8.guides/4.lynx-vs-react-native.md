...
Lynx is not quite a year and a half into public life. The core is production-proven inside ByteDance at a scale most applications will never reach, third-party native modules are scarce, and component libraries number in the low single digits. Those are the official headless [`@lynx-js/lynx-ui`](https://github.com/lynx-family/lynx-ui) for ReactLynx and [Vy UI](/getting-started) for Vue Lynx, compared in the [libraries guide](/guides/lynx-ui-frameworks).

The layer above the runtime has started to fill in. TikTok open-sourced [Sparkling](/guides/sparkling) in November 2025, which supplies the native shell that Lynx itself leaves out, meaning containers, scheme-based routing between pages, and a code-generated JavaScript-to-native bridge. React Native has had that tier since Expo, so this is the ecosystem catching up rather than pulling ahead.
