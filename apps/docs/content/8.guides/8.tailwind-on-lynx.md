...
The Lynx side cannot fix that alone. Either Tailwind reintroduces a way to restrict the emitted utility set, or the preset post-processes the generated CSS to strip what the engine will not understand, and both are real work rather than a version bump. [lynx-stack#69](https://github.com/lynx-family/lynx-stack/issues/69), open since March 2025, is where v4 support for the preset is tracked.

The layer underneath the preset moves on its own schedule, and the two are easy to conflate. [rsbuild-plugin-tailwindcss#17](https://github.com/rstackjs/rsbuild-plugin-tailwindcss/issues/17) covers Tailwind v4 in the build plugin rather than in the preset, and its maintainer rewrote that plugin for v4 on a `v4` branch in April 2026, after which Rsbuild shipped `@rsbuild/plugin-tailwindcss` built on Tailwind's own webpack loader instead of PostCSS. Rspeedy 0.16.1 depends on `@rsbuild/core` 2.1.7, so that plugin is version-compatible with a Lynx project already. What it solves is getting v4 through the bundler, which is a different question from the 122 core plugins the Lynx CSS engine does not implement, and that second question is the one the preset answers.
