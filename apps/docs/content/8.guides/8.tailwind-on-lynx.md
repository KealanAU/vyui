...
The Lynx side cannot fix that alone. Either Tailwind reintroduces a way to restrict the emitted utility set, or the preset post-processes the generated CSS to strip what the engine will not understand, and both are real work rather than a version bump. [lynx-stack#69](https://github.com/lynx-family/lynx-stack/issues/69) has tracked v4 support for the preset since March 2025 and is still open, with the last substantive comment in September of that year.

The layer underneath the preset has moved, which is easy to mistake for progress on this. [rsbuild-plugin-tailwindcss#17](https://github.com/rstackjs/rsbuild-plugin-tailwindcss/issues/17) covers Tailwind v4 in the build plugin rather than in the preset, and its maintainer rewrote that plugin for v4 on a `v4` branch in April 2026, after which Rsbuild shipped `@rsbuild/plugin-tailwindcss` built on Tailwind's own webpack loader instead of PostCSS. Rspeedy 0.16.1 depends on `@rsbuild/core` 2.1.7, so that plugin is version-compatible with a Lynx project already. What it solves is getting v4 through the bundler, and it says nothing about the 122 core plugins the Lynx CSS engine cannot render, which is the job the preset exists to do.

This answer has a shelf life, so check both issues before assuming v4 is still blocked.
...
Verified against `@lynx-js/tailwind-preset@0.5.0` and `tailwindcss@4.3.0` on 25 July 2026. The preset's latest npm release is still 0.5.0, published on 17 June 2026 and still peering on `tailwindcss@^3.4.0`. [lynx-stack#69](https://github.com/lynx-family/lynx-stack/issues/69) and [rsbuild-plugin-tailwindcss#17](https://github.com/rstackjs/rsbuild-plugin-tailwindcss/issues/17) track any movement.
::
