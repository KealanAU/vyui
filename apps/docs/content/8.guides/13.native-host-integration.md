...
seo:
  ...
  description: The global props and global events a Lynx host app should inject — safe-area insets for the notch and Dynamic Island, the device theme, and live appearance changes — with Swift and Kotlin snippets.
---
...
With that in place, `useColorMode()` in `'system'` mode boots on the device appearance and follows it live — [`VyApp`](/components/app) re-skins the tree through its existing remount contract, nothing extra to wire in the bundle.
...
`themechanged` is nothing special — it is one name Vy UI happens to listen for. Your host can send anything through the same channel (connectivity, deep links, push payloads), and the bundle subscribes with [`useGlobalEvent`](/composables/use-global-event):
