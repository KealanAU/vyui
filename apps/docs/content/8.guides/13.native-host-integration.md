...
**Use [Sparkling](/guides/sparkling) instead.** Sparkling injects safe-area insets, theme, and global events into the Lynx runtime for you, so the manual wiring on this page is only needed if you are building a custom host from scratch. For most apps, `npx sparkling run:ios` replaces everything below.
::
