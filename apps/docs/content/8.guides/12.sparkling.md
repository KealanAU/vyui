...
Sparkling is the recommended way to ship a Vue Lynx app to native. Building the host yourself — what our [Xcode guide](/guides/testing-with-xcode) walks through — is the fallback for when you need full control over the container or want to understand the seam from first principles. For everything else, Sparkling's containers, routing, bridge, and dev workflow are already written and maintained by the Lynx team, so there is rarely a reason to rebuild them.
...
The repository has been public since late November 2025 and is under active development, so expect the surface to keep settling. Nothing in the design is React-specific past the templates, and the rough edges a Vue project meets are the same ones a React project meets. The [Sparkling documentation](https://tiktok.github.io/sparkling/) covers the full API surface, and is the reference for anything beyond this overview.
