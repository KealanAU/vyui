...
Lynx ships a renderer and a bundler and stops there. `rspeedy build` produces a `main.lynx.bundle`, Lynx Go loads it from a QR code, and everything past that is yours to write: the Activity or view controller that hosts a `LynxView`, the mechanism that opens a second page, and every call that crosses from JavaScript into native code. Sparkling is TikTok's answer to that gap, open-sourced in November 2025 under Apache-2.0 and described in its own documentation as unlocking "Lynx at TikTok scale".
...
It is young, with the repository public since late November 2025, and the surface is broad enough that some of it will move. Nothing about the design is Vue-hostile, and the parts most likely to bite are the same parts that bite React users.
