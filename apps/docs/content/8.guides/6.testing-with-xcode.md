...
seo:
  ...
  description: A working iOS host app for Vue Lynx, covering the XcodeGen spec, CocoaPods pin set, Swift bootstrap, dev-server discovery, and the Release path that embeds the bundle.
---
...
**[Sparkling](/guides/sparkling) is the recommended path for a native host.** It ships the iOS and Android shell, containers, routing, a code-generated native bridge, and `npx sparkling run:ios` — everything below, already written. This guide remains for cases where you need full control over the container or want to understand the Lynx host seam from first principles.
::
...
::card-group
  ...
  to: /guides/sparkling
  ---
  The recommended native shell — do this first unless you need full control.
  :::
