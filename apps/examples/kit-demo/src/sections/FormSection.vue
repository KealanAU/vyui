...
<template>
  <view class="flex flex-col gap-4 pt-2">
    ...
    <view class="bg-default border border-default rounded-lg p-4 flex flex-col gap-3">
      <text class="text-highlighted text-base font-semibold">Button</text>
    ...
    <!-- Button variants -->
    <view class="bg-default border border-default rounded-lg p-4 flex flex-col gap-3">
      <text class="text-highlighted text-base font-semibold">Button variants</text>

      <text class="text-muted text-xs">solid</text>
      <view class="flex flex-row flex-wrap gap-2">
    ...
    <view class="bg-default border border-default rounded-lg p-4 flex flex-col gap-2">
      <text class="text-highlighted text-base font-semibold">Input / Label</text>
      ...
      />
      <text v-if="name" class="text-muted text-xs pt-1">Hello, {{ name }}!</text>
    </view>

    <!-- Input focus ring -->
    <view class="bg-default border border-default rounded-lg p-4 flex flex-col gap-2">
      <text class="text-highlighted text-base font-semibold">Input focus ring</text>
      <text class="text-muted text-xs">Tap an input — colored border + shadow ring should follow focus, and drop on blur.</text>
    ...
    <view class="bg-default border border-default rounded-lg p-4 flex flex-col gap-2">
      <text class="text-highlighted text-base font-semibold">Keyboard avoidance — off</text>
    ...
    <view class="bg-default border border-default rounded-lg p-4 flex flex-col gap-2">
      <text class="text-highlighted text-base font-semibold">Keyboard avoidance — on</text>
      <text class="text-muted text-xs">
    ...
    <!-- Textarea -->
    <view class="bg-default border border-default rounded-lg p-4 flex flex-col gap-2">
      <text class="text-highlighted text-base font-semibold">Textarea</text>
      <VyTextarea v-model="bio" placeholder="Tell us about yourself" :rows="3" />
      <text v-if="bio" class="text-muted text-xs pt-1">{{ bio.length }} chars</text>
    </view>

    <!-- Switch -->
    <view class="bg-default border border-default rounded-lg p-4 flex flex-col gap-4">
      <text class="text-highlighted text-base font-semibold">Switch</text>
      <view class="flex flex-row items-center justify-between">
        <view class="flex flex-row items-center gap-2">
          <VyIcon name="icon-park-outline:wifi" :size="18" color="#0f172a" />
          <text class="text-highlighted text-sm">Wi-Fi</text>
        </view>
      ...
      <view class="flex flex-row items-center justify-between">
        <view class="flex flex-row items-center gap-2">
          ...
          <text class="text-highlighted text-sm">Bluetooth</text>
        </view>
    ...
    <!-- Checkbox -->
    <view class="bg-default border border-default rounded-lg p-4 flex flex-row items-center gap-3">
      <VyCheckbox v-model="agreed" />
      <text class="text-highlighted text-sm">I agree to the terms and conditions</text>
    </view>

    <!-- RadioGroup -->
    <view class="bg-default border border-default rounded-lg p-4 flex flex-col gap-3">
      <text class="text-highlighted text-base font-semibold">RadioGroup</text>
      <VyRadioGroup v-model="radioPlan" :items="radioItems" />
      <text class="text-muted text-xs">Selected: {{ radioPlan }}</text>
    </view>

    <!-- ToggleGroup -->
    <view class="bg-default border border-default rounded-lg p-4 flex flex-col gap-3">
      <text class="text-highlighted text-base font-semibold">ToggleGroup</text>
      <VyToggleGroup v-model="toggleAlign" :items="toggleItems" type="single" />
      <text class="text-muted text-xs">Align: {{ toggleAlign }}</text>
    </view>
    ...
    <!-- NumberField -->
    <view class="bg-default border border-default rounded-lg p-4 flex flex-col gap-3">
      <text class="text-highlighted text-base font-semibold">NumberField</text>

      <view class="flex flex-col gap-2">
        <VyLabel for="qty-field">Quantity (0–10)</VyLabel>
        <VyNumberField id="qty-field" v-model="quantity" :min="0" :max="10" :step="1" />
        <text class="text-muted text-xs">Value: {{ quantity ?? '—' }}</text>
      </view>
      ...
      <view class="flex flex-col gap-2 pt-1">
        ...
        <VyNumberField
          ...
          placeholder="0.00"
        />
        <text class="text-muted text-xs">Value: {{ price ?? '—' }}</text>
      </view>
    ...
    <!-- Stepper -->
    <view class="bg-default border border-default rounded-lg p-4 flex flex-col gap-3">
      <text class="text-highlighted text-base font-semibold">Stepper</text>
    ...
    <!-- PinInput -->
    <view class="bg-default border border-default rounded-lg p-4 flex flex-col gap-3">
      <text class="text-highlighted text-base font-semibold">PinInput</text>
      <VyPinInput v-model="pin" :length="6" />
      <text class="text-muted text-xs">Entered: {{ pin || '—' }}</text>
    </view>

    <!-- Form + FormField (sync validators, label/description/error scaffold) -->
    <view class="bg-default border border-default rounded-lg p-4 flex flex-col gap-3">
      <text class="text-highlighted text-base font-semibold">Form + FormField</text>
      <VyForm
        ref="formRef"
        v-slot="{ submitting }"
