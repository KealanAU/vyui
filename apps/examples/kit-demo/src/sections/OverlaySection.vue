...
<template>
  <view class="flex flex-col gap-4 pt-2">
    <view class="bg-default border border-neutral-200 rounded-lg p-4 flex flex-col gap-2">
      <text class="text-neutral-900 text-base font-semibold">Modal</text>
      <text class="text-neutral-500 text-xs">Dialog with overlay backdrop.</text>
      <VyModal v-model:open="modalOpen" title="Modal title" description="A centered dialog with an overlay backdrop.">
        ...
        <template #content>
          <view class="flex flex-col gap-3 p-4">
            <text class="text-neutral-600 text-sm">
    ...
    <view class="bg-default border border-neutral-200 rounded-lg p-4 flex flex-col gap-2">
      <text class="text-neutral-900 text-base font-semibold">Tray</text>
        ...
        <template #default="{ setView, goBack, canGoBack }">
          <VyTrayView id="menu">
            <view class="flex flex-col gap-2">
              <text class="text-neutral-900 text-base font-semibold">Quick actions</text>
          ...
          <VyTrayView id="share">
            <view class="flex flex-col gap-2">
              ...
              <text class="text-neutral-900 text-base font-semibold">Share</text>
              <text class="text-neutral-500 text-sm">A taller view — the tray grows to fit it.</text>
              <text class="text-neutral-600 text-sm">Anyone with the link can view. Toggle access, copy the URL, or invite by email — plenty of room here so the panel is noticeably taller than the confirm view.</text>
            </view>
          </VyTrayView>
          ...
          <VyTrayView id="confirm">
            <view class="flex flex-col gap-2">
              <VyButton v-if="canGoBack" color="neutral" variant="ghost" size="sm" label="← Back" @tap="goBack()" />
              <text class="text-neutral-900 text-base font-semibold">Delete item?</text>
              <text class="text-neutral-500 text-sm">A short view — the tray shrinks down.</text>
            </view>
          </VyTrayView>
        </template>
    ...
    <view class="bg-default border border-neutral-200 rounded-lg p-4 flex flex-col gap-2">
      <text class="text-neutral-900 text-base font-semibold">Keyboard-aware tray</text>
      <text class="text-neutral-500 text-xs">
      ...
      <VyTray v-model:open="kbScrollOpen" keyboard-aware :ui="{ bodyScroll: 'max-h-80' }">
        <view class="flex flex-col gap-3">
          <text class="text-neutral-900 text-base font-semibold">Edit profile</text>
          <text class="text-neutral-400 text-xs">@keyboard: {{ kbLast }}</text>
      ...
      <VyTray v-model:open="kbLiftOpen" keyboard-aware="lift">
        <view class="flex flex-col gap-2">
          <text class="text-neutral-900 text-base font-semibold">Quick reply</text>
          <text class="text-neutral-400 text-xs">@keyboard: {{ kbLast }}</text>
    ...
    <view class="bg-default border border-neutral-200 rounded-lg p-4 flex flex-col gap-2">
      <text class="text-neutral-900 text-base font-semibold">Notifications</text>
      <text class="text-neutral-500 text-xs">
      ...
      <view class="flex flex-col gap-2">
        <view
          ...
          :key="n.id"
          class="flex flex-row items-center gap-3 px-3 py-2.5 bg-neutral-50 border border-neutral-200 rounded-lg"
        >
          <view class="w-8 h-8 rounded-full bg-default border border-neutral-200 flex flex-row items-center justify-center shrink-0">
            <text class="text-neutral-700 text-base">•</text>
          </view>
          <view class="flex flex-col flex-1 min-w-0 gap-0.5">
            <text class="text-neutral-900 text-sm font-medium truncate">{{ n.title }}</text>
            <text class="text-neutral-500 text-xs truncate">{{ n.summary }}</text>
          </view>
    ...
    <view class="bg-default border border-neutral-200 rounded-lg p-4 flex flex-col gap-2">
      <text class="text-neutral-900 text-base font-semibold">Stacked toasts (Sonner)</text>
      <text class="text-neutral-500 text-xs">
    ...
    <view class="bg-default border border-neutral-200 rounded-lg p-4 flex flex-col gap-2">
      <text class="text-neutral-900 text-base font-semibold">Progress color</text>
      <text class="text-neutral-500 text-xs">
    ...
    <view class="bg-default border border-neutral-200 rounded-lg p-4 flex flex-col gap-2">
      <text class="text-neutral-900 text-base font-semibold">Drawer</text>
      <text class="text-neutral-500 text-xs">
    ...
    <view class="bg-default border border-neutral-200 rounded-lg p-4 flex flex-col gap-2">
      <text class="text-neutral-900 text-base font-semibold">Ghost-close regression</text>
      <text class="text-neutral-500 text-xs">
      ...
      >
        ...
        <template #body>
          <view class="flex flex-col gap-2 px-4 py-2">
            <text class="text-neutral-900 text-2xl font-semibold">Ticks: {{ ghostTicks }}</text>
            <text class="text-neutral-500 text-xs">
    ...
    <view class="bg-default border border-neutral-200 rounded-lg p-4 flex flex-col gap-2">
      <text class="text-neutral-900 text-base font-semibold">DropdownMenu</text>
      <text class="text-neutral-500 text-xs">Tap-anchored menu.</text>
    ...
    <view class="bg-default border border-neutral-200 rounded-lg p-4 flex flex-col gap-3">
      <text class="text-neutral-900 text-base font-semibold">Select</text>
      <text class="text-neutral-500 text-xs">Bottom-sheet picker — items pop up from below. This covers the listbox pattern too (no separate Listbox component).</text>
      <VySelect v-model="selectCountry" :items="countryItems" placeholder="Pick a country" />
      <text class="text-neutral-500 text-xs">Country: {{ selectCountry }}</text>
    </view>

    <view class="bg-default border border-neutral-200 rounded-lg p-4 flex flex-col gap-3">
      <text class="text-neutral-900 text-base font-semibold">Combobox / Autocomplete</text>
      <text class="text-neutral-500 text-xs">Typeahead picker — the in-sheet search filters items as you type. This is the autocomplete pattern (no separate component).</text>
      <VyCombobox v-model="comboboxFruit" :items="fruitItems" placeholder="Pick a fruit" search-placeholder="Search fruits…" />
      <text class="text-neutral-500 text-xs">Fruit: {{ comboboxFruit }}</text>
    </view>
  </view>
